# Primeval Monsters

Big Hit Points, Big Damage, Low Chance To Hit

## Razor Lizard

3 Pack

Hit Points: d6/2
Evade: 6
Limit: 10

- Attacks
  - Minimum Damage 2
  - If there are as many Razor Lizards as targets, attack with Advantage

## Herd Lizard

Hit Points: 20 +d6
Evade: 3
Limit: 11

- Movement
  - Charge! Movement is an Attack at Disadvantage
- Attack
  - Swipe! Attack 2 Different Targets Critical Hit +d6\*

## Flying Lizard

Hit Points: 15 + d6
Evade: 6
Limit: 10

- Movement
  - At Pocket 1 it's Grounded.
  - Recovers Move after Attack if it's In The Air
- Attacks
  - If it Moved before the Attack then it Pierces with Beak for Strong Hit +2
  - Attack 2x with Claws Minimum Damage 2

Hint: Probably need something like Ranged or a Net

## Long Neck Lizard

Hit Points: 40 +5d6
Evade: 3
Limit: 11

- Movement
  - Charge! Movement is an Attack at Disadvantage
- Attacks
  -Move -> Attack then flies in with beak poke Strong Hit +2 Damage
  -Grounded Claws -> 2x Attack

Attacks an entire area
Any Hit Moves Target 1, +d6* damage
Critical Hit +2d6* more damage
